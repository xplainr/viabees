...
return new Response(JSON.stringify({ status: "valid", campaign: metadata.campaign }), {
  headers: { "Content-Type": "application/json" },
    ...
    if (pathname === "/api/share" && request.method === "POST") {
      ...
      const campaignRes = await fetch(`${env.SUPABASE_URL}/rest/v1/campaign?id=eq.${campaign_id}&select=business_id,recipient_offer_text`, {
        headers: {
          apikey: env.SUPABASE_SERVICE_ROLE,
          Authorization: `Bearer ${env.SUPABASE_SERVICE_ROLE}`
        }
      ...
      // KV write
      await env.CLAIM_KV.put(claim_token, JSON.stringify({
